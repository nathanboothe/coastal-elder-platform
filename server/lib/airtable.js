...
/**
 * Gets a single record by ID — used for ownership checks (e.g. confirming
 * an 'elder'-role user isn't deleting someone else's availability/time-off
 * record before allowing the delete).
 */
async function getRecord(tableName, recordId) {
  return airtableFetch(`${encodeURIComponent(tableName)}/${recordId}`);
}

module.exports = { listRecords, createRecord, createRecords, updateRecords, deleteRecords, getRecord };